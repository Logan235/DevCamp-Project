...
export class RoadmapService {
  ...
  private buildRoadmapNodes(
    ...
    for (const group of groupedChallenges) {
      ...
      // Filter out challenges that have already been used in previous nodes
      const availableChallenges = group.challenges.filter((challenge) => {
      ...
      // Chunk the available challenges into multiple arrays of max size maxExercisesPerNode
      const chunks: LeanChallenge[][] = [];
      for (let i = 0; i < availableChallenges.length; i += maxExercisesPerNode) {
        chunks.push(availableChallenges.slice(i, i + maxExercisesPerNode));
      }
      ...
      for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
        ...
        const nodeChallenges = chunks[chunkIndex];

        // Mark these challenges as used
        for (const challenge of nodeChallenges) {
        ...
        let patternTitle = this.getPatternDisplayName(group.patternGroup);
        if (chunks.length > 1) {
          patternTitle += ` - Phần ${chunkIndex + 1}`;
        }

        nodes.push({
          order: nodeNumber,
          title: `${nodeNumber}. ${patternTitle}`,
  ...
  private groupChallengesByPattern(
    ...
    for (const challenge of challenges) {
      ...
      // Group by skill + pattern to keep related patterns separate
      const groupKey = `${skillSlug || 'general'}_${patternGroup}`;
